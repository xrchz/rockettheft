...
- `correct_fee_recipient`: Boolean indicating whether the fee recipient for this block (according to either the relay's payload if `mev_reward` is present, or the Beacon chain otherwise) was correct (either the smoothing pool if `in_smoothing_pool` is `true`, or the node's fee recipient otherwise). Note that this matches neither of the potential sources of truth [Redstone update](https://docs.rocketpool.net/guides/redstone/whats-new.html#the-penalty-system) or [penalty system draft](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system.md#penalizable-conditions), which disagree. We are happy to rerun once there is a clear definition. 
